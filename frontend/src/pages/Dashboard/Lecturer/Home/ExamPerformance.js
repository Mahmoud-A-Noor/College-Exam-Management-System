...
export default function ExamPerformance({examPerformanceData}){

  useEffect(() => {
    const data = examPerformanceData.datasets[0].data;
    ...
    const colors = examPerformanceData.datasets[0].data.map((value) => {
    ...
    examPerformanceData.datasets[0].backgroundColor = colors;
    // eslint-disable-next-line
}, [])
  ...
  const options = {
    ...
    return(
        <div>
            ...
            <Bar data={examPerformanceData} options={options} />
        </div>
    )
}